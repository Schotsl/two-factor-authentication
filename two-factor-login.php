...
class Simba_Two_Factor_Authentication
{
    ...
    public function __construct()
    {
        ...
        if (file_exists(SIMBA_TFA_PLUGIN_DIR.'/premium.php')) {
            include_once(SIMBA_TFA_PLUGIN_DIR.'/premium.php');
        }
        ...
        if (file_exists(SIMBA_TFA_PLUGIN_DIR.'/updater.php')) {
            include_once(SIMBA_TFA_PLUGIN_DIR.'/updater.php');
        }
    ...
    public function getTFA()
    {
        if (!class_exists('HOTP')) {
            require_once(SIMBA_TFA_PLUGIN_DIR.'/hotp-php-master/hotp.php');
        }
        if (!class_exists('Base32')) {
            require_once(SIMBA_TFA_PLUGIN_DIR.'/Base32/Base32.php');
        }
        if (!class_exists('Simba_TFA')) {
            require_once(SIMBA_TFA_PLUGIN_DIR.'/includes/class-simba-tfa.php');
        }
    ...
    public function tfaInitLogin()
    {
        ...
        if (defined('TWO_FACTOR_DISABLE') && TWO_FACTOR_DISABLE) {
            ...
            if (!empty($_COOKIE['simbatfa_trust_token'])) {
                $auth_info['trust_token'] = (string) $_COOKIE['simbatfa_trust_token'];
            }
        ...
        if (!empty($res['user_can_trust'])) {
            ...
            if (!empty($res['user_already_trusted'])) {
                $results['user_already_trusted'] = 1;
            }
        }
    ...
    public function tfaVerifyCodeAndUser($user, $username, $password)
    {
        ...
        if (is_wp_error($user)) {
            ...
            if (!empty($_COOKIE['simbatfa_trust_token'])) {
                $params['trust_token'] = (string) $_COOKIE['simbatfa_trust_token'];
            }
    ...
    public function print_private_keys($admin, $type = 'full', $user_id = false)
    {
        ...
        if (!$tfa_priv_key_64) {
            $tfa_priv_key_64 = $tfa->addPrivateKey($user_id);
        }
		...
		<div class="postbox" style="clear:both;">
			...
			<?php if ($admin) { ?>
				<h3 style="padding: 10px 6px 0px; margin:4px 0 0; cursor: default;">
                    ...
                    if ($current_user->ID == $user_id) {
                        echo $this->reset_current_otp_link();
                    } ?>
					</span>
					<div class="inside">
			...
			<?php } else {
				...
				<div class="inside">
					...
					<?php
                    $time_now = time();
                        $refresh_after = 30 - ($time_now % 30); ?><span class="simba_current_otp" data-refresh_after="<?php echo $refresh_after; ?>"><?php print $tfa->generateOTP($user_id, $tfa_priv_key_64); ?></span>
    ...
    {
        if (!class_exists('TFA_Frontend')) {
            require_once(SIMBA_TFA_PLUGIN_DIR.'/includes/tfa_frontend.php');
        }
        if (empty($this->frontend)) {
            $this->frontend = new TFA_Frontend($this);
        }
